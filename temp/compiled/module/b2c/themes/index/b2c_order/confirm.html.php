...
<html>
	...
	<body>
		...
		<div class="main-body" id="app" v-if="pageLoad" style="display:none">
			<form id="orderForm">
				<input type="hidden" name="ispin" value="<?php echo $this->_var['ispin']; ?>" />
				<input type="hidden" name="pin_orderid" value="<?php echo $this->_var['pin_orderid']; ?>" />
			...
			<?php if ($this->_var['vipcard']): ?>
			<div class="row-box mgb-5">
				VIP会员卡：余额 ￥<?php echo $this->_var['vipcard']['money']; ?>元
			</div>
			<?php endif; ?>
		...
		<script>
			var ispin="<?php echo $this->_var['ispin']; ?>"
		</script>
		<script src="<?php echo $this->_var['skins']; ?>b2c_order/confirm.js"></script>
		<script>
			$(function(){
